...
class PhaseSystemVerilog(pc: PhaseContext, report: SpinalReport[_]) extends PhaseMisc with VerilogBase {
  import pc._

  var outFile: java.io.FileWriter = null
  def targetPath = pc.config.targetDirectory + "/" +  (if(pc.config.netlistFileName == null)(topLevel.definitionName + ".sv") else pc.config.netlistFileName)
  ...
  override def impl(pc: PhaseContext): Unit = {
    ...
    if (!pc.config.oneFilePerComponent) {
      ...
      for (c <- sortedComponents) {
        ...
        if (!moduleContent.contains("replaced by")) {
          val targetFilePath = pc.config.targetDirectory + "/" +  (if(pc.config.netlistFileName == null)(c.definitionName + ".sv") else pc.config.netlistFileName)
  ...
  def compile(component: Component): () => String = {
    val componentBuilderSystemVerilog = new ComponentEmitterSystemVerilog(
      c                           = component,
      systemVerilog               = true,
      verilogBase                 = this,
      algoIdIncrementalBase       = allocateAlgoIncrementaleBase,
    ...
    if(component.parentScope == null && pc.config.dumpWave != null) {
      componentBuilderSystemVerilog.logics ++=
        s"""
  initial begin
    $$dumpfile("${pc.config.dumpWave.vcdPath}");
    $$dumpvars(${pc.config.dumpWave.depth}, ${component.definitionName});
...
"""
    ...
    val trace = componentBuilderSystemVerilog.getTrace()
    val oldComponent = emitedComponent.getOrElse(trace, null)

    if (oldComponent == null) {
      emitedComponent += (trace -> component)
      () => componentBuilderSystemVerilog.result
    } else {
      emitedComponentRef.put(component, oldComponent)
      component.definitionName = oldComponent.definitionName
      () => s"\n//${component.definitionName} replaced by ${oldComponent.definitionName}\n"
